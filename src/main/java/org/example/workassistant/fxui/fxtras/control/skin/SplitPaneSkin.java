package org.example.workassistant.fxui.fxtras.control.skin;

import org.example.workassistant.fxui.fxtras.control.SplitPane;
...
//        control.setManaged(false);
            ...
            for (int i = 0; i < contentRegions.size(); i++) {
                ...
                if (i < contentDividers.size()) {
                    ...
                    if (divider.posExplicit) {
                        checkDividerPosition(divider, posToDividerPos(divider, divider.d.getPosition()), divider.getDividerPos());
                    }
                    if (i == 0) {
                        // First panel
    ...
    private void checkDividerPosition(ContentDivider divider, double newPos, double oldPos) {
        ...
        double maxLeft = left == null ? 0 : left.getContent() != null ? (horizontal) ? left.getContent().maxWidth(-1) : left.getContent().maxHeight(-1) : 0;
        double maxRight = right == null ? 0 : right.getContent() != null ? (horizontal) ? right.getContent().maxWidth(-1) : right.getContent().maxHeight(-1) : 0;
...
//            System.out.println("LAYOUT " + c.getId() + " PANELS X " + c.getX() + " Y " + c.getY() + " W " + (horizontal ? c.getArea() : width) + " H " + (horizontal ? height : c.getArea()));
            if (horizontal) {
                ...
                layoutInArea(c, c.getX() + paddingX, c.getY() + paddingY, c.getArea(), height, 0/*baseline*/, HPos.CENTER, VPos.CENTER);
            } else {
                c.setClipSize(width, c.getArea());
                layoutInArea(c, c.getX() + paddingX, c.getY() + paddingY, width, c.getArea(), 0/*baseline*/, HPos.CENTER, VPos.CENTER);
            }
        }
        for (ContentDivider c : contentDividers) {
